...
class BertForSequenceClassification(nn.Module):
    ...
    def __init__(self, num_labels):
        ...
        self.pos_embed = nn.Embedding(17,768,padding_idx = 0)
        self.dis_embed = nn.Embedding(128,50,padding_idx = 0)
       ...
       # temp = torch.nn.init.orthogonal_(embedding_weight)
        ...
        self.wp_d = nn.Linear(768,768)
        self.wp_c = nn.Linear(768,768)
        self.wd_d = nn.Linear(50,768)
        self.wd_c = nn.Linear(50,768)
    ...
    def load_dis_embed(self):
        dis_embed = np.zeros((128,50))
        for i in range(1,128):
            dis_embed[i] = 1.0/ np.exp(i)
        dis_embed = torch.tensor(dis_embed,dtype = torch.float)
        return dis_embed
    def forward(self, input_ids, token_type_ids, attention_mask, class_labels, detection_lablels,aspects,pos,dis):
...
#        pos_embed = self.embedding(pos_embed)
       ...
       # full_pos_embed = pos_embed.expand(-1,128,768)
        Md = self.wh_d(encode)+self.wa_d(full_aspect_embed)+ self.wd_d(dis_embed)#+(pos_embed)
        ...
        Mc = self.wh_c(encode)+self.wa_c(full_aspect_embed)+ self.wd_c(dis_embed)#+(pos_embed)
