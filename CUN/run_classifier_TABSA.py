...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id, label_exist_id, aspect):
...
def main():
    ...
    train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all__detection_label_ids,all_aspects)
    ...
    if args.eval_test:
        ...
        test_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all_detection_label_ids,all_aspects)
   ...
   # model = torch.load("model_data/attention_add_model")
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            input_ids, input_mask, segment_ids, label_ids, exist_ids, all_aspects = batch
            loss_1, _ = model(input_ids, segment_ids, input_mask, label_ids, exist_ids, all_aspects,0)
            loss_2, _ = model(input_ids, segment_ids, input_mask, label_ids, exist_ids, all_aspects,1)
            loss = loss_1 + loss_2
            if n_gpu > 1:
        ...
        if args.eval_test :
            ...
            with open(os.path.join(args.output_dir, "test_ep_"+str(epoch)+".txt"),"w") as f_test:
                for input_ids, input_mask, segment_ids, label_ids, exist_ids,all_aspects in test_dataloader:
                    ...
                    with torch.no_grad():
                        tmp_test_loss, detect_logits = model(input_ids, segment_ids, input_mask, label_ids, exist_ids,all_aspects,0)
                        tmp_test_loss_2,sent_logits = model(input_ids, segment_ids, input_mask, label_ids, exist_ids,all_aspects,1)
                    tmp_test_loss += tmp_test_loss_2
